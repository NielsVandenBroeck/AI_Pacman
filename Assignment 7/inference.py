...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        #handle jail position first
        if ghostPosition == jailPosition:
            return noisyDistance is None
        #check if noisydistance is none
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        position is known.
        """
        for tile in self.allPositions:
            self.beliefs[tile] *= self.getObservationProb(observation,gameState.getPacmanPosition(),tile,self.getJailPosition())
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for tile in self.allPositions:
            belief = self.beliefs[tile]
            newPosDist = self.getPositionDistribution(gameState,tile)
            for possibility in newPosDist.keys():
                newBeliefs[possibility] += belief * newPosDist[possibility]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        particlesPerTile = int(self.numParticles/len(self.legalPositions))
        particlesLeft = self.numParticles%len(self.legalPositions)
        for position in self.legalPositions:
            self.particles += [position]*(particlesPerTile + 1) if  particlesLeft > 0 else [position]*particlesPerTile
            particlesLeft -= 1
    ...
    def elapseTime(self, gameState):
        ...
        for particle in range(self.numParticles):
            ...
            self.particles[particle] = newPosDist.sample()
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        positionsLeft = self.numParticles%numPositions
        while len(self.particles) + numPositions < self.numParticles:
            self.particles += positions
        self.particles += positions[:positionsLeft]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                prob *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), particle[i],
